...
BEGIN
  ...
  WITH matched AS (
    SELECT
      ...
      -- Relevancia: email exacto > nombre empieza por > contiene > empresa/tel/cif
      CASE
        ...
        WHEN lower(e.nombre) LIKE '%' || normalized_query || '%' THEN 6
        WHEN lower(e.cif) LIKE '%' || normalized_query || '%' THEN 7
        ELSE 8
      END AS relevance
    FROM contactos c

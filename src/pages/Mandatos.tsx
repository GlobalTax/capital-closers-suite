...
export default function Mandatos() {
  ...
  const allColumnDefinitions = useMemo(() => ({
    ...
    empresas: {
      ...
      sortable: false,
      render: (value: any[], row: Mandato) => {
        // Para mandatos de compra, mostrar indicador para expandir
        if (row.tipo === 'compra') {
          ...
          return (
            <div className="flex items-center gap-2 text-muted-foreground">
              ...
              <span className="text-sm font-medium text-foreground">{targets.length}</span>
        ...
        <div className="flex-1 min-w-0">
          ...
          {vistaActual === "tabla" && (
            <DataTableEnhanced
              ...
              isRowExpandable={(row) => 
                row.tipo === 'compra' && 
                (row.empresas?.filter((e: any) => e.rol === 'target').length || 0) > 0
              }
              renderExpandedRow={(row) => {
                const targets = row.empresas?.filter((e: any) => e.rol === 'target') || [];
                return <TargetsSublist targets={targets} />;
              }}
            />
          )}
